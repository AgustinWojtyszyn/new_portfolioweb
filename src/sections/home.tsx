...
import { identity, type Language } from '@/lib/site';
...
export function Hero({lang}:{lang:Language}) {
 ...
 return <section className="hero container" aria-labelledby="hero-title"><div className="hero-top mono"><span>{t.eyebrow}</span><span className="hero-coordinate">31°32′ S &nbsp; 68°32′ W</span></div><div className="hero-grid"><div className="hero-copy"><p className="overline"><i className="status-dot"/>{t.role}</p><h1 id="hero-title">{t.hero[0]}<br/>{t.hero[1]}<br/><em>{t.hero[2]}</em></h1><p className="hero-intro">{t.intro}</p><div className="hero-actions"><a className="button-primary" href="#work">{t.work}<span>↘</span></a><a className="text-link" href={identity.github} target="_blank" rel="noreferrer">GitHub ↗</a><a className="text-link" href={identity.linkedin} target="_blank" rel="noreferrer">LinkedIn ↗</a></div></div><SystemMap lang={lang}/></div><div className="hero-bottom mono"><span>{t.location} <span className="muted">/ UTC−3</span></span><span className="hero-bottom-center">INTERFACE → OPERATION → SYSTEM</span><a href="#work" aria-label={t.work}>SCROLL TO EXPLORE <span>↓</span></a></div></section>;
}
export function SelectedWork({lang}:{lang:Language}) {
 const t=getContent(lang), flagship=projects[0];
 return <section id="work" className="section container"><div className="section-heading"><p className="eyebrow">01 / {t.selected}</p><h2>{t.selectedSub}</h2><span className="index-stamp mono">SELECTED<br/>2024—2026</span></div><article className="featured-project"><div className="featured-copy"><span className="eyebrow">{flagship.number} / {flagship[lang].category}</span><h3>{flagship.name}</h3><p className="project-headline">{flagship[lang].headline}</p><p className="body-copy">{flagship[lang].summary}</p><p className="tech-line mono">React · Supabase · PostgreSQL</p><Link className="text-link arrow-link" href={`/${lang}/work/${flagship.slug}`}>{t.case}<span>↗</span></Link></div><Link href={`/${lang}/work/${flagship.slug}`} className="project-visual-link" aria-label={`${t.case}: ${flagship.name}`}><OrderVisual lang={lang}/></Link></article><div className="project-list">{projects.slice(2).map(p=><Link className="project-row" key={p.slug} href={`/${lang}/work/${p.slug}`}><span className="project-number mono">{p.number}</span><div><span className="eyebrow">{p[lang].category}</span><h3>{p.name}</h3><p>{p[lang].summary}</p></div><span className="project-row-stack mono"><ProjectSignal slug={p.slug}/>{p.stack.slice(0,3).join(' / ')}</span><span className="row-arrow">↗</span></Link>)}</div></section>;
}
export function Ecosystem({lang}:{lang:Language}) {
 const t=getContent(lang);
 return <section className="ecosystem section"><div className="container ecosystem-grid"><div><p className="eyebrow">{t.ecosystemLabel}</p><h2>{t.ecosystemTitle}</h2><p className="body-copy">{t.ecosystemText}</p><p className="ecosystem-note mono">↳ {t.ecosystemFoot}</p></div><div className="ecosystem-network"><div className="network-root"><span className="mono">SERVIFOOD</span><span className="network-plus">+</span><strong>{lang==='es'?'Operación real':'Real operations'}</strong></div><div className="network-systems">{[projects[0],projects[3],projects[2]].map(p=><Link href={`/${lang}/work/${p.slug}`} key={p.slug}><i className="status-dot"/><span>{p.short}</span><span>↗</span></Link>)}</div><div className="network-base mono"><span>Corporate Web</span><span>Operational Data</span></div></div></div></section>;
}
export function Midnight({lang}:{lang:Language}) {
 const t=getContent(lang);
 return <section id="midnight" className="midnight section"><div className="container"><div className="midnight-title"><div><p className="eyebrow">02 / {t.midnightLabel}</p><h2>MIDNIGHT<br/><span>PASS</span><sup>↗</sup></h2></div><div className="midnight-intro"><p>{t.midnightText}</p><Link href={`/${lang}/work/midnight-pass`} className="text-link arrow-link">{t.case}<span>↗</span></Link></div></div><Road lang={lang}/><div className="midnight-spec mono"><span>REACT THREE FIBER</span><span>RAPIER PHYSICS</span><span>PROCEDURAL WORLD</span><span>3 CAMERAS</span></div></div></section>;
}
...
export function Stack({lang}:{lang:Language}) {
 const t=getContent(lang);
 return <section id="stack" className="section stack-section"><div className="container"><div className="split-heading"><div><p className="eyebrow">04 / {t.stackLabel}</p><h2>{t.stackTitle}</h2></div><p className="body-copy">{t.stackText}</p></div><div className="stack-grid">{stack.map((s,i)=><article className="stack-item" key={s.title}><div className="stack-item-top"><span className="mono">0{i+1}</span><span aria-hidden="true">↘</span></div><h3>{s.title}</h3><p className="mono stack-tools">{s.tools}</p><p>{s[lang]}</p></article>)}</div><div className="stack-pipeline mono">React <span>→</span> Auth <span>→</span> PostgreSQL <span>→</span> RLS <span>→</span> RPC <span>→</span> Edge Functions <span>→</span> Render</div></div></section>;
}
